...
class UserRepositoryImpl (
    ...
    override suspend fun insertUser(userData: UserData, password: String): DataLayerResponse<Long> {
        val userEntity = userData.toUserEntity(password)
    ...
    override suspend fun updateUser(userData: UserData): DataLayerResponse<Boolean> {
    ...
    override suspend fun deleteUser(userData: UserData): DataLayerResponse<Boolean> {
        val userEntity = userData.toUserEntity()
    ...
    override suspend fun getFriendsOfUser(userId: Long): DataLayerResponse<List<UserProfileSummary>> {
        ...
        userDao.getFriendsOfUser(userId).forEach {
            val profilePath = File(userProfileImageFileDir,"${it.userId}${StorageHelper.IMAGE_TYPE_JPG}").absolutePath
            userProfileSummaryList.add(it.toUserProfileSummery(profilePath))
        }
    ...
    override suspend fun getUserProfileSummeryByUserId(
        ...
        return userDao.getUserById(userId).run {
            if (this != null) {
                val profilePath = File(userProfileImageFileDir,"$userId${StorageHelper.IMAGE_TYPE_JPG}").absolutePath
                DataLayerResponse.Success(this.toUserProfileSummery(profilePath))
            } else DataLayerResponse.Error(DataLayerErrorCode.OPERATION_FAILED)
        }
    }
    ...
    override suspend fun getAllUsers(): DataLayerResponse<List<UserProfileSummary>> {
        return userDao.getAllUsers().run {
            ...
            this.forEach {
                ...
                val profilePath = File(userProfileImageFileDir,"${it.userId}${StorageHelper.IMAGE_TYPE_JPG}").absolutePath
                list.add(it.toUserProfileSummery(profilePath))
            }
    ...
    override suspend fun authenticateUser(phone: Long): DataLayerResponse<Pair<UserProfileSummary, String>> {
        ...
        return if (userEntity != null) {
            val profilePath = File(userProfileImageFileDir,"${userEntity.userId}${StorageHelper.IMAGE_TYPE_JPG}").absolutePath
            val user = userEntity.toUserProfileSummery(profilePath)
