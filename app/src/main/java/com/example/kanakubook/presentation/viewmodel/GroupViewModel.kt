...
class GroupViewModel(
    ...
    private val _getAllGroupExpenseResponse =
    ...
    val getGroup = MutableLiveData<GroupData?>()
    ...
    fun getGroupByGroupId(groupId: Long?) {
        viewModelScope.launch(Dispatchers.IO) {
            if (groupId == null) {
                getGroup.postValue(null)
                return@launch
            }
            when (val data = useCase.getGroupByGroupId(groupId)) {
                is PresentationLayerResponse.Success -> {
                    getGroup.postValue(data.data)
                }
    ...
    fun addProfile(groupId: Long, bitmap: Bitmap) {
        viewModelScope.launch(Dispatchers.IO) {
            profilePictureUseCase.addProfileImage(ImageDirectoryType.Group(groupId), bitmap)
        }
    }
